...
def load_egg_price_history():
    ...
    date_candidates = ["date", "observation_date", "Date", "DATE"]
    price_candidates = ["egg_price", "value", "Value", "APU0000708111"]

    date_column = next(
        (column for column in date_candidates if column in history.columns),
        None,
    )
    price_column = next(
        (column for column in price_candidates if column in history.columns),
        None,
    )

    if date_column is None:
        raise ValueError("No recognized date column was found.")
    if price_column is None:
        raise ValueError("No recognized egg-price column was found.")

    history = history.rename(
        columns={date_column: "date", price_column: "egg_price"}
    )
    history["date"] = pd.to_datetime(history["date"], errors="coerce")
    history["egg_price"] = pd.to_numeric(
        history["egg_price"], errors="coerce"
    )
    history = history.dropna(subset=["date", "egg_price"])
...
with st.sidebar:
    ...
    st.write(
        "A machine learning application that estimates the average "
    ...
    st.caption(f"Loaded model: {type(model).__name__}")
...
st.markdown(
    """
    <div class="subtitle">
        Estimate the monthly average retail price of one dozen Grade A large
        eggs using current economic and agricultural conditions.
    </div>
    """,
    unsafe_allow_html=True,
)
...
prediction_tab, dashboard_tab, model_tab, project_tab = st.tabs(
    [
        "🔮 Make a Prediction",
        "📊 Data Dashboard",
        "🌲 Model Details",
...
with prediction_tab:
    ...
    st.write(
        "Corn and soybean prices are entered in dollars per bushel. "
        "The application converts them to dollars per metric ton before "
        "making a prediction."
    )
    ...
    with st.form("prediction_form"):
        ...
        with right_column:
            inflation_rate = st.number_input(
                ...
                value=3.0,
                step=0.1,
                help="Enter 3.2 to represent an inflation rate of 3.2%.",
            )
    ...
    if submit_button:
        bird_flu_outbreak = 1 if outbreak_choice == "Yes" else 0
        ...
        try:
            ...
            prediction = float(model.predict(new_data_prepared)[0])
        except Exception as error:
        ...
        with prediction_box:
            st.metric(
                ...
                help=(
                    "Estimated average retail price for one dozen "
                    "Grade A large eggs."
                ),
            )
            st.caption("Per dozen Grade A large eggs")
        ...
        with result_column_2:
            st.metric("Outbreak Status", outbreak_choice)
        with result_column_3:
            st.metric("Inflation", f"{inflation_rate:.2f}%")
...
This application uses a **Tuned Random Forest machine learning model**
trained on historical U.S. egg prices and related economic and
agricultural indicators.

### Please note

- Predictions are **estimates**, not guaranteed future prices.
- The model learns statistical patterns from historical data.
- Predictions may not always align with expected economic trends.
- Results should **not** be used for financial, agricultural,
  investment, or business decisions.

This project was developed for **educational and portfolio purposes**
to demonstrate machine learning, data analysis, and web application
deployment using Python, Scikit-learn, and Streamlit.
                """
            )
        ...
        csv_data = prediction_download.to_csv(index=False).encode("utf-8")
        st.download_button(
        ...
        if bird_flu_outbreak == 1:
            insights.append(
                "🦠 A bird flu outbreak was reported. Reduced poultry "
                "supply may place upward pressure on egg prices."
            )
        if inflation_rate >= 4:
            insights.append(
                "📈 Inflation is relatively high. Production, packaging, "
                "and transportation costs may be elevated."
            )
        elif inflation_rate < 0:
            insights.append(
                "📉 Inflation is negative, which may indicate slower "
                "overall price growth."
            )
        if diesel_price >= 4:
            insights.append(
                "⛽ Diesel prices are elevated. Higher transportation "
                "costs may influence retail egg prices."
            )
        if corn_bushel >= 6:
            insights.append(
                "🌽 Corn prices are above the typical range. Higher feed "
                "costs may increase production expenses."
            )
        if soybean_bushel >= 14:
            insights.append(
                "🌱 Soybean prices are relatively high. This may increase "
                "poultry feed costs."
            )
        if bird_affected >= 10_000_000:
            insights.append(
                "🐔 A large number of birds were affected. This may reduce "
                "egg production and tighten supply."
            )
        elif 0 < bird_affected < 10_000_000:
            insights.append(
                "🐔 Bird flu affected part of the poultry population, "
                "which may influence supply."
            )
        if prediction >= 5:
            insights.append(
                "💵 The predicted egg price is relatively high compared "
                "with many historical periods."
            )
        elif prediction <= 2:
            insights.append(
                "💵 The predicted egg price is relatively low compared "
                "with many recent periods."
            )
        if not insights:
            insights.append(
                "✅ The entered conditions appear relatively stable. "
                "No major high-cost or outbreak signals were detected."
            )
        ...
        st.caption(
            "These AI insights are rule-based observations created from "
            "the entered values. They do not show the exact internal "
            "reasoning of the Random Forest and do not prove causation."
        )
...
with dashboard_tab:
    ...
    st.write(
        "Explore historical egg prices and examine which features were "
    ...
    if egg_history is not None and not egg_history.empty:
        chart_data = egg_history.set_index("date")[["egg_price"]]
        st.line_chart(
            chart_data,
            x_label="Date",
        ...
        latest_price = egg_history.iloc[-1]["egg_price"]
        highest_price = egg_history["egg_price"].max()
        average_price = egg_history["egg_price"].mean()
        latest_date = egg_history.iloc[-1]["date"].strftime("%B %Y")

        price_column_1, price_column_2, price_column_3 = st.columns(3)
        with price_column_1:
            st.metric(
        ...
        with price_column_2:
            st.metric("Historical Average", f"${average_price:.2f}")
        with price_column_3:
            st.metric("Highest Historical Price", f"${highest_price:.2f}")

        with st.expander("View historical egg-price data"):
            history_table = egg_history.copy()
            history_table["date"] = history_table["date"].dt.strftime(
                "%B %Y"
            )
            history_table["egg_price"] = history_table["egg_price"].round(2)
            history_table = history_table.rename(
                columns={"date": "Month", "egg_price": "Egg Price ($)"}
            )
            history_table = history_table.iloc[::-1]
            st.dataframe(
                history_table,
                hide_index=True,
                use_container_width=True,
        ...
        st.warning("Historical egg-price data could not be loaded.")
        if "history_error" in globals():
            st.code(history_error)

    st.divider()
    st.subheader("Tuned Random Forest Feature Importance")
    ...
    if hasattr(model, "feature_importances_"):
        ...
        feature_chart = feature_importance.set_index("Feature")[
            ["Importance (%)"]
        ]
        st.bar_chart(
            feature_chart,
        ...
        display_importance = feature_importance.sort_values(
            by="Importance (%)",
            ascending=False,
        )
        st.dataframe(
            display_importance[["Feature", "Importance (%)"]],
            hide_index=True,
            use_container_width=True,
        )
        ...
        st.info(
            ...
            f"**{most_important['Feature']}**, with an importance score "
            f"of **{most_important['Importance (%)']:.2f}%**."
        )
        st.caption(
            "Feature importance measures predictive usefulness inside "
            "the model. It does not prove causation or show whether a "
            "feature raises or lowers the prediction."
        )
    else:
        st.warning("The loaded model does not provide feature importance.")
...
with model_tab:
    ...
    with model_column_1:
        st.metric("Mean Absolute Error", "0.127")
    with model_column_2:
        st.metric("Root Mean Squared Error", "0.308")
    with model_column_3:
    ...
    st.write(
        """
        The application uses a Tuned Random Forest Regressor trained on
        historical economic and agricultural data. A Random Forest combines
        predictions from many decision trees. The final result is based on the
        average prediction across those trees.

        The model learns statistical patterns from the training data. It does
        not apply economic rules, so raising every input does not guarantee a
        higher predicted egg price.
        """
    )
    ...
    st.warning(
        "The model uses historical relationships and does not include every "
        "possible cause of egg-price changes. Weather, labor costs, consumer "
        "demand, government policy, reporting gaps, and supply-chain events "
        "may also affect actual prices."
    )
...
with project_tab:
    ...
    st.write(
        """
        The goal of this project was to develop a machine learning system
        that predicts the monthly average U.S. retail price of one dozen
        Grade A large eggs.
        """
    )
